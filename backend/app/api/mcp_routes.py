...
async def process_text_with_mcp(request: MCPTextRequest):
    ...
    try:
        ...
        try:
            ...
            logger.warning(f"⚠️ MCP API: Invalid intent '{request.intent}', defaulting to comprehensive_processing")
...
async def delete_document(document_id: str, user_email: str = Header(alias="user-email")):
    ...
    try:
        logger.info(f"🗑️ Deleting document: {document_id} for user: {user_email}")
        ...
        try:
            ...
            if hash_ids:
                ...
                logger.info("⚠️ No DocumentHashes records found for document")
            
        except Exception as spanner_error:
            logger.warning(f"⚠️ Could not delete from Spanner: {str(spanner_error)}")
