...
Intelligent routing layer that orchestrates between:
...
class MCPServer:
    ...
    async def route_request(self, intent: ProcessingIntent, **kwargs) -> MCPToolResult:
        ...
        try:
            ...
            if intent == ProcessingIntent.DOCUMENT_ANALYSIS:
                ...
                logger.info("⚙️ MCP SERVER: Handling COMPREHENSIVE_PROCESSING workflow")
    ...
    async def _handle_document_analysis(self, file_content: bytes, mime_type: str, 
        ...
        try:
            ...
            try:
                ...
                logger.warning(f"⚠️ MCP: Blockchain security failed, continuing without: {blockchain_error}")
    ...
    async def _handle_legal_query(self, text: str, user_email: str = None, context: str = None) -> MCPToolResult:
        ...
        try:
            ...
            if response:
                ...
                logger.warning("⚠️ MCP LEGAL QUERY: Failed to generate response")
                return MCPToolResult(
                    success=False,
    ...
    async def _handle_comprehensive_processing(self, text: str, user_email: str = None) -> MCPToolResult:
        ...
        """
        try:
            logger.info("⚙️ MCP COMPREHENSIVE: Starting comprehensive document analysis")
            ...
            for term, gemini_definition in complex_terms.items():
                ...
                if spanner_definition:
                    ...
                    logger.info(f"⚠️ MCP COMPREHENSIVE: Term '{term}' not in Spanner, using Gemini definition")
                    enhanced_terms.append({
                        'term': term,
                        'definition': gemini_definition,
                        'source': 'gemini_ai',
            ...
            logger.info(f"🗄️ MCP COMPREHENSIVE: {spanner_matches} terms from Spanner database")
